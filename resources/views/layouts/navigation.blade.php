            ...
            <div class="mr-4 flex items-center">
                ...
                <a class="flex items-center px-2 py-2 text-gray-500 hover:text-blue-600" href="">
                    ...
                    <span class="absolute -mt-5 ml-4 flex">
                        <span
                            class="absolute inline-flex h-3 w-3 animate-ping rounded-full bg-blue-400 opacity-75"></span>
                ...
                <!-- Profile -->
                @if (Auth::check())
                <div class="relative ml-2 inline-block" x-data="{ openProfileDropdown: false }"
                    @click.away="openProfileDropdown = false">

                    <!-- Dropdown toggle button -->
                    <button
                        class="relative z-10 flex items-center rounded-md border bg-white p-2 text-sm text-gray-600 focus:border-blue-500 focus:outline-none"
                    ...
                    <!-- Dropdown menu -->
                    <div class="absolute right-0 z-20 mt-2 w-64 origin-top-right overflow-hidden rounded-md bg-white p-2 py-2 shadow-xl focus:outline-none"
                        x-show="openProfileDropdown" x-transition:enter="transition ease-out duration-100"
                        ...
                        <div class="flex items-center gap-4 rounded-lg p-2 font-normal duration-300 hover:bg-gray-100">
                            <img class="mx-1 h-9 w-9 flex-shrink-0 rounded-full object-cover"
                                src="{{ asset(auth()->check() ? 'images/profile/avatar/' . auth()->user()->profile_picture : '') }}"
                                alt="" loading="lazy">
                            <div class="font-medium">
                                <div> {{ auth()->check() ? auth()->user()->name : '' }} </div>
                                <div class="text-xs text-gray-500">{{ auth()->check() ? auth()->user()->email : '' }}
                                </div>
                            </div>
                        </div>
                        ...
                        <ul class="space-y-2">
                            <li>
                                <a class="flex items-center rounded-lg p-2 text-base font-normal text-gray-900 hover:bg-gray-100"
                                    href="{{ route('profile') }}">
                                    <svg class="h-6 w-6 text-gray-400" fill="none" stroke-linecap="round"
                            ...
                            <li>
                                <form class="flex items-center rounded-lg p-2 text-base font-normal hover:bg-gray-100"
                                    action="{{ route('logout') }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <svg class="h-6 w-6 text-red-400" fill="none" stroke-linecap="round"
                                        stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24"
                                        stroke="currentColor">
                                        <path
