    ...
    <div class="flex h-auto w-full flex-col items-center justify-center">
        ...
        <div class="mt-4 flex h-auto w-full flex-row flex-wrap items-center justify-center gap-3">
            ...
            <a href="{{ route('addresses.create') }}">
                <div class="flex h-96 w-80 flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-300">
                ...
                @foreach ($addresses as $address)
                    @if ($address->is_default)
                        <div class="relative flex h-96 w-80 flex-col justify-start rounded-lg border-2 border-gray-300">
                            <div class="absolute left-0 top-0 flex w-full flex-row justify-start rounded-t-lg border-2 border-gray-300 p-2">
                                <p class="text-md font-semibold">Default: </p>
                                <p class="ml-2 text-lg font-semibold text-blue-600">{{ $address->name }}</p>
                            </div>
                            ...
                            <!-- Bottom Buttons -->
                            <div class="absolute inset-x-0 bottom-0 flex h-16 w-full flex-row justify-end space-x-4 p-2">
                                <a href="{{ route('addresses.edit', $address->id) }}">
                                    <p class="text-lg text-blue-600">Edit</p>
                                </a>
                                <x-neomorphic-form action="{{ route('addresses.destroy', $address->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button class="text-lg text-blue-600" type="submit">Remove</button>
                                </x-neomorphic-form>
                            </div>
                        </div>
                    @endif
