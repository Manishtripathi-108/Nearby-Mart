        ...
        <div class="flex w-full flex-col flex-wrap gap-x-6 gap-y-10 py-10 pr-6">
            ...
            <x-neomorphic-form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="my-4 flex h-auto w-full flex-row items-center justify-center space-x-2">
                    @php
                        $images = [['id' => 'main_image', 'name' => 'photo_main', 'label' => 'Product Image 1', 'src' => 'https://via.placeholder.com/150'], ['id' => 'photo1_image', 'name' => 'photo_1', 'label' => 'Product Image 2', 'src' => 'https://via.placeholder.com/150'], ['id' => 'photo2_image', 'name' => 'photo_2', 'label' => 'Product Image 3', 'src' => 'https://via.placeholder.com/150']];
                    @endphp
                ...
                <div class="m-4">
                    ...
                    <x-neomorphic-form.input class="w-full" id="name" name="name" type="text" value="{{ old('name') }}" />
                    @error('name')
                        <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex w-full flex-row items-center justify-center space-x-2">
                    <div class="mb-4 w-full">
                        ...
                        <x-neomorphic-form.input class="w-full" id="price" name="price" type="number" value="{{ old('price') }}" step="0.01" />
                        @error('price')
                            <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4 w-full">
                        ...
                        <x-neomorphic-form.input class="w-full" id="discount" name="discount" type="number" value="{{ old('discount') }}" step="0.01" />
                        @error('discount')
                            <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mb-4">
                    ...
                    <x-neomorphic-form.textarea class="w-full" id="description" name="description">{{ old('description') }}</x-neomorphic-form.textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex w-full flex-row items-center justify-center space-x-2">
                    <div class="mb-4 w-full">
                        ...
                        <x-neomorphic-form.input class="w-full" id="stock" name="stock" type="number" value="{{ old('stock') }}" />
                        @error('stock')
                            <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4 w-full">
                        ...
                        <x-neomorphic-form.input class="w-full" id="measure" name="measure" type="number" value="{{ old('measure') }}" />
                        @error('measure')
                            <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4 w-full">
                        ...
                        <x-neomorphic-form.select class="w-full" id="sold_by" name="sold_by">
                            @foreach ($soldBy as $value)
                                <option value="{{ $value }}" {{ old('sold_by') == $value ? 'selected' : '' }}>{{ $value }}</option>
                            @endforeach
                        </x-neomorphic-form.select>
                        @error('sold_by')
                            <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
